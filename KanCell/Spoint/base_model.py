...
#     def forward(self, x):
#         batch_size, seq_len, hidden_dims = x.size()
...
#         # Concatenate heads
#         out = out.transpose(1, 2).contiguous().view(batch_size, seq_len, hidden_dims)
...
#         nn.init.kaiming_normal_(self.encoder[0].weight) # type: ignore
#         nn.init.kaiming_normal_(self.encoder[4].weight) # type: ignore
#         nn.init.kaiming_normal_(self.decoder[0].weight) # type: ignore
#         nn.init.kaiming_normal_(self.decoder[3].weight) # type: ignore
#         nn.init.kaiming_normal_(self.decoder[6].weight) # type: ignore
#         nn.init.xavier_uniform_(self.decoder[-1].weight) # type: ignore
#         nn.init.kaiming_normal_(self.pred[0].weight) # type: ignore
#         nn.init.xavier_uniform_(self.pred[4].weight) # type: ignore
...
#         )
#         self.decoder = nn.Sequential(
#             ResidualBlock(celltype_dims, hidden_dims, dropout),  # Use ResidualBlock here
#             ResidualBlock(hidden_dims, hidden_dims, dropout),    # Use ResidualBlock here
#             ResidualBlock(hidden_dims, hidden_dims, dropout),    # Use ResidualBlock here
#             nn.Linear(hidden_dims, input_dims)
#         )
#         self.pred = nn.Sequential(
...
#         nn.init.kaiming_normal_(self.decoder[0].fc1.weight)
#         nn.init.kaiming_normal_(self.decoder[1].fc1.weight)
#         nn.init.kaiming_normal_(self.decoder[2].fc1.weight)
...
#             nn.LayerNorm(hidden_dims),
#             nn.Dropout(dropout),
#             ResidualBlock(hidden_dims, hidden_dims, dropout),
#             GaussianNoise(noise_stddev),  # Add Gaussian noise here
#             nn.Linear(hidden_dims, latent_dims),
#         )
#         self.decoder = nn.Sequential(
#             ResidualBlock(celltype_dims, hidden_dims, dropout),  # Use ResidualBlock here
...
#         self.pred = nn.Sequential(
#             # nn.Linear(latent_dims, hidden_dims),
#             KAN.KANLinear(latent_dims, hidden_dims),
#             nn.LeakyReLU(),
#             nn.LayerNorm(hidden_dims),
#             nn.Dropout(dropout),
#             # nn.Linear(hidden_dims, celltype_dims),
#             KAN.KANLinear(hidden_dims, celltype_dims),
#             nn.Softmax(dim=1),
#         )
#         for layer in self.encoder:
#             if isinstance(layer, nn.Linear):
